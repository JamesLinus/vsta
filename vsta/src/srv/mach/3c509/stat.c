 ...
 */
void
el3_stat(struct msg *m, struct file *f)
{
	char buf[MAXSTAT];
	uint len;
	struct attach *o;
	...
	strcat(buf, perm_print(o ? (&o->a_prot) : &el3_prot));
	m->m_buf = buf;
 ...
 */
void
el3_wstat(struct msg *m, struct file *f)
{
	struct attach *o;
	char *field, *val;
	 ...
	 */
	if (do_wstat(m, &el3_prot, f->f_perm, &field, &val) == 0) {
		return;
	}
