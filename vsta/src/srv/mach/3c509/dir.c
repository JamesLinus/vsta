 ...
 */
void
el3_readdir(struct msg *m, struct file *f)
{
	char *buf;
	uint len, pos, bufcnt;
	struct llist *l;
 ...
 */
void
el3_open(struct msg *m, struct file *f)
{
	struct attach *o;
	uint x;
	char *p = m->m_buf;
	...
	if (!o) {
		...
		ASSERT_DEBUG(o->a_refs > 0, "el3_open: overflow");
...
el3_close(struct file *f)
{
	 ...
	 */
	ASSERT_DEBUG(o->a_refs > 0, "el3_close: underflow");
	o->a_refs -= 1;
	if (o->a_refs == 0) {
		freeup(o);
		return;
