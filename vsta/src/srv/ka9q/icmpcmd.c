...
static void del_ping();
static struct ping *add_ping();
static int16 hash_ping();
static int pingem();
...
char *argv[];
{
	...
	if (argc > 2) {
		...
		if(pp == NULLPING)
			pp = add_ping(dest);
		pp->timer.start = atoi(argv[2]) * (1000 / MSPTICK);
...
struct icmp *icmp;
{
	...
	rtt = mark_msec() - icmp->args.echo.seq;
