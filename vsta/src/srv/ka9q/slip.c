...
char **argv;
{
	...
	while(argc) {
		...
		if (m_send(dev,argv[i++]) == -1)
			return -1;
		if (--argc > 0) {
			if (m_expect(dev,argv[i++]) == -1)
				return -1;
			argc--;
		}
	}
...
doslipstat()
{
	...
	for(ifp=ifaces;ifp != NULLIF;ifp = ifp->next){
	  ...
	  if (sp->vjcomp) {
	    printf("  Input:");
	    slhc_i_status((struct slcompress *)sp->slcomp);
	    printf("  Output:");
	    slhc_o_status((struct slcompress *)sp->slcomp);
	  }
	}
}
