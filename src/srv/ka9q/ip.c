...
void ip_recv();
static void freefrag(), ip_timeout(), free_reasm();
static struct frag *newfrag();
static struct reasm *creat_reasm(), *lookup_reasm();
...
/* 
 * the TCP will just append the bytes to the file.  If we don't
 * have the number of bytes we think we do, and the extras
 * happen to be zero, the checksum test won't trigger, and we'll
 * lose.
 */
		if (len_mbuf(bp) != length) {
			free_p(bp);
			return;
		}
